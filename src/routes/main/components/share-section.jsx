...
const ShareCard = ({ title, description, img }) => {
  return (
    <div className="flex flex-col rounded-xl shadow-md w-full md:max-w-[450px] max-h-[378px] overflow-hidden">
      <img src={img} alt={title} className="w-full object-cover" />
      <div className="p-4 md:p-5 flex flex-col items-start gap-1 md:gap-1.5">
        <h4 className="text-sm md:text-base font-normal text-neutral-800">{title}</h4>
        <p className="text-lg md:text-xl font-extrabold text-neutral-800">{description}</p>
      </div>
    </div>
  );
};
